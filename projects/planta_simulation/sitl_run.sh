...
if [ "$program" == "jmavsim" ] && [ ! -n "$no_sim" ]; then
	...
	if [ -x "$(command -v gazebo)" ]; then
		...
		while gz model --verbose --spawn-file="${modelpath}/${model}/${model_name}.sdf" --model-name=${model} -x ${origin_x} -y ${origin_y} -z ${origin_z} -Y ${origin_yaw} 2>&1 | grep -q "An instance of Gazebo is not running."; do
